        ...
        def fill_template(template, **kwargs):
            if len(env_settings) > 0 and "env_settings_lines" not in kwargs:
                kwargs["env_settings_lines"] = "\n".join(
                    [f'Environment="{sym}={val}"' for sym, val in env_settings.items()]
                )
            else:
                kwargs["env_settings_lines"] = ""
            ...
            if template_found:
                ...
                if len(services_to_enable) > 0:
                    ...
                    if have_dbus:
                        ...
                        logger.warning(
                            "No dbus client library could be loaded.  Install service files manually."
                        )
                    return
...
setup(
    ...
    data_files=[
        ...
        (
            ...
            ["postfix/outbound_quota/main.cf", "postfix/outbound_quota/master.cf"],
        ),
        (
    ...
    scripts=[*SERVICES, "services/null_filter.py", "install/chapps_database_init.py"],
    version=verstr,
    license="MIT",
