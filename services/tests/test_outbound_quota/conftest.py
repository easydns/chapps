...
from chapps.tests.test_policy.conftest import clear_redis, populate_redis, well_spaced_attempts, rapid_attempts
from services.tests.conftest import test_message_factory, mail_sink, known_sender, test_recipients

@fixture(scope='session')
def chapps_oqp_service(request, run_services, mail_sink,
        chapps_mock_session,
    ...
    """
    if run_services:
        chapps_watcher = watcher_getter( "./services/chapps_outbound_quota.py",
...
def oqp_test_message_factory():
    return test_message_factory( "CHAPPS-OQP",
        "CHAPPS automated outbound quota policy service testing" )
