...
def test_chapps_obm_sda_rej(
    ...
    message = obm_test_message_factory(known_sender, obm_test_recipients)

    with SMTP("127.0.0.1") as smtp:
        with pytest.raises(SMTPRecipientsRefused):
            assert smtp.sendmail(known_sender, obm_test_recipients, message)


def test_chapps_obm_auth_send(
    caplog,
    chapps_obm_service,
    known_sender,
    obm_test_recipients,
    ...
    userauth_queries = [
        f"INSERT INTO domains ( name ) VALUES ('{known_sender.split('@')[1]}');",
    ...
    with SMTP("127.0.0.1") as smtp:
        smtp.sendmail(known_sender, obm_test_recipients, message)
    assert True  # success if the email gets sent


def test_chapps_obm_denied_unknown(
    caplog,
    chapps_obm_service,
    unknown_sender,
    obm_test_recipients,
    ...
    """
    caplog.set_level(logging.DEBUG)
    message = obm_test_message_factory(unknown_sender, obm_test_recipients)
    with SMTP("127.0.0.1") as smtp:
        with pytest.raises(SMTPRecipientsRefused):
            assert smtp.sendmail(unknown_sender, obm_test_recipients, message)


def test_chapps_obm_denied_overquota(
    caplog,
    chapps_obm_service,
    known_sender,
    obm_test_recipients,
...
def test_chapps_obm_denied_spammy(
    caplog,
    chapps_obm_service,
    known_sender,
    obm_test_recipients,
