...
from services.tests.conftest import test_message_factory, test_recipients, known_sender, mail_sink
...
def grl_test_message_factory():
    return test_message_factory( "CHAPPS-GRL",
                                 "CHAPPS automated greylisting policy service testing" )
...
@fixture(scope="session")
def chapps_grl_service(request, run_services, mail_sink,
                       chapps_mock_session,
                       chapps_mock_config_file,
                       watcher_getter):
    """Load the greylisting service using the mock-config"""
    if run_services:
        chapps_watcher = watcher_getter( "./services/chapps_greylisting.py",
...
@fixture(scope="session")
def chapps_grl_service_with_tuple(chapps_grl_service, known_sender, grl_test_recipients):
