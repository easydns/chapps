...
from chapps.tests.test_policy.conftest import auto_spf_query, mock_spf_queries, _auto_query_param_list, idfn, testing_policy_spf
...
class Test_SPFEnforcementHandler:
    ...
    async def test_handle_policy_request(self, caplog, monkeypatch,
                                         testing_policy_spf, auto_spf_query, expected_result,
                                         mock_reader_factory, mock_writer,
    ):
        """
        ...
        handle_spf_request = SPFEnforcementHandler( testing_policy_spf ).async_policy_handler()
        ...
        mock_writer.write.assert_called_with( f"action={expected_result}\n\n".encode('utf-8') )
