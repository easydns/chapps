...
import pytest
import mariadb
from chapps.adapter import PolicyConfigAdapter, MariaDBQuotaAdapter, MariaDBSenderDomainAuthAdapter
...
class Test_MariaDBQuotaAdapter():
    ...
    def test_quota_for_user( self,
            populated_database_fixture, finalizing_mdbqadapter):
        """
    ...
    def test_quota_for_nonexistent_user( self,
            populated_database_fixture, finalizing_mdbqadapter):
        """
    ...
    def test_quota_dump( self,
        ...
        assert { r[0] for r in results } == { 'ccullen@easydns.com',
                                              'somebody@chapps.io',
                                              'bigsender@chapps.io' }
        assert { r[1] for r in results } == { 240, 1200, 4800 }

    def test_quota_search( self,
            populated_database_fixture,
            finalizing_mdbqadapter,
            test_emails
    ):
    ...
    def test_quota_search_accuracy( self,
            populated_database_fixture,
            finalizing_mdbqadapter,
            test_emails
    ):
    ...
    def test_MDBQA_quota_dict( self,
            populated_database_fixture,
            finalizing_mdbqadapter,
            test_emails
    ):
        ...
        assert results == { 'ccullen@easydns.com': 240,
                            'somebody@chapps.io': 1200 }

    def test_quota_map( self,
            populated_database_fixture,
            finalizing_mdbqadapter,
            test_emails,
            mock_mapping
        ...
        results = finalizing_mdbqadapter.quota_map(
            mock_mapping, # the mock mapping function, which we can query
            test_emails ) # the emails to test with
    ...
    def test_quota_map_noncallable( self,
            populated_database_fixture,
            finalizing_mdbqadapter
    ):
        """
        ...
        """
        with pytest.raises(ValueError, match='must be a callable which accepts the user and quota as arguments'):
...
class Test_MariaDBSenderDomainAuthAdapter():
    ...
    def test_check_domain_for_user( self, finalizing_mdbsdaadapter, populated_database_fixture ):
        ...
        """
        assert finalizing_mdbsdaadapter.check_domain_for_user( "ccullen@easydns.com", "chapps.io" )

    def test_check_domain_for_unauth_user( self, finalizing_mdbsdaadapter, populated_database_fixture ):
        """GIVEN a user has an entry in the database
           WHEN  that user tries to send an email for a domain they are not linked to
           THEN  return a False result
        """
        assert not finalizing_mdbsdaadapter.check_domain_for_user( "ccullen@easydns.com", "example.com" )

    def test_check_domain_for_nonexistent_user( self, finalizing_mdbsdaadapter, populated_database_fixture ):
        """GIVEN a user has no entry in the database
           WHEN  that user tries to send an email for any domain
           THEN  return a False result
        """
        assert not finalizing_mdbsdaadapter.check_domain_for_user( "nonexistent@example.com", "chapps.io" )
