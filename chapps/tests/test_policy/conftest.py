...
from chapps.tests.test_config.conftest import chapps_mock_config, chapps_mock_env, chapps_mock_cfg_path, chapps_mock_config_file
from chapps.tests.test_util.conftest import postfix_policy_request_message, postfix_policy_request_payload
...
from chapps.tests.conftest import _unique_instance, _mock_client_tally, _clear_redis, _populate_redis_grl
import redis
import time
import random
import string
...
def sda_allowable_ppr( postfix_policy_request_message ):
    return OutboundPPR( postfix_policy_request_message( 'caleb@chapps.io', sasl_username='ccullen@easydns.com', ccert_subject='' ))

@fixture
def sda_unauth_ppr( postfix_policy_request_message ):
    return OutboundPPR( postfix_policy_request_message( 'ccullen@easydns.com', sasl_username='somebody@chapps.io', ccert_subject='' ))
...
def multisend_ppr_factory(random_recipient, postfix_policy_request_message):
    def _ppr_factory(sender, recipient_count):
        return OutboundPPR(
            postfix_policy_request_message(
                sender,
                [ random_recipient for _ in range( recipient_count ) ]
            )
        )
    return _ppr_factory
...
def random_recipient():
    return ''.join( random.choice(string.ascii_letters) for _ in range(8) ) + "@recipient.com"
...
def well_spaced_attempts():
    ...
    def _wsa( number ):
        ...
        return [ t0 + float( t ) + ( float(delta) * random.random() ) for t in range(0, int( seconds_per_day ), delta) ]
    return _wsa
...
def populate_redis(clear_redis):
    ...
    def _popredis( email, limit, timestamps=[], margin=0 ):
        ...
        with rh.pipeline() as pipe:
            pipe.delete( fmtkey( email, 'limit'), fmtkey( email, 'attempts'), fmtkey( email, 'margin') )
...
def _mock_spf_query(result, message):
    ...
    mock.get_header = Mock( name='get_header', return_value='X-CHAPPSTESTING: SPF prepend' )
    return mock
...
def no_helo_softfail_mf():
    return mock_spf_queries(
        ('none', ''),
        ('softfail', 'CHAPPS softfail SPF message')
    )
