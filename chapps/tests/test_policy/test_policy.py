...
from chapps.policy import EmailPolicy, GreylistingPolicy, OutboundQuotaPolicy, SenderDomainAuthPolicy
from chapps.config import CHAPPSConfig
from chapps.tests.test_config.conftest import chapps_mock_cfg_path, chapps_mock_env, chapps_mock_config, chapps_mock_config_file, chapps_sentinel_cfg_path, chapps_sentinel_env, chapps_sentinel_config, chapps_sentinel_config_file
...
class Test_EmailPolicy():
    ...
    def test_connect_to_sentinel( self, chapps_sentinel_env, chapps_sentinel_config_file ):
        """
...
class Test_GreylistingPolicyEvaluation():
    ...
    def test_recognized_tuple_passes(self, caplog, monkeypatch, allowable_ppr):
        ...
        monkeypatch.setattr( policy, '_get_control_data', lambda x: (time.time() - (60*15), None) )
        response = policy._evaluate_policy_request( allowable_ppr )
        assert response == True

    def test_recognized_tuple_updates_client_tally(self, caplog, monkeypatch, allowable_ppr):
        """
        GIVEN a recognized tuple - a timestamp
        WHEN  the tuple was seen more than min_delay seconds ago
        THEN  return True
    ...
    def test_sufficient_client_tally_permits_sending_for_unrecognized_tuple(self,
                                                                            caplog,
                                                                            monkeypatch,
                                                                            allowable_ppr,
                                                                            mock_client_tally,
        ...
        with monkeypatch.context() as m:
            ...
            populate_redis_grl( policy.tuple_key( ppr ),
                                { policy.client_key( ppr ):
                                  mock_client_tally( policy.allow_after ) } )
        response = policy._evaluate_policy_request( ppr )
        assert response == True

    def test_client_tally_updated_when_unrecognized_tuple_passes(self,
                                                                 caplog,
                                                                 monkeypatch,
                                                                 allowable_ppr,
                                                                 mock_client_tally,
...
class Test_GreylistingPolicy_update_control_data():
    ...
    def test_skip_client_tally_update_if_allow_after_is_zero(self,
                                                             caplog,
                                                             monkeypatch,
                                                             clear_redis_grl,
                                                             allowable_ppr
...
class Test_GreylistingPolicy_get_control_data():
    ...
    def test_allow_after_is_zero(self, caplog, monkeypatch, clear_redis_grl, allowable_ppr):
        """
    ...
    def test_new_tuple_client_tally_present(self, caplog, monkeypatch, mock_client_tally,
                                            populate_redis_grl, allowable_ppr, unique_instance):
        """
    ...
    def test_tuple_recognized_and_tally_exists(self, caplog, mock_client_tally,
                                               allowable_ppr, populate_redis_grl, unique_instance):
        """
...
class Test_OutboundQuotaPolicy():
    ...
    def test_approve_policy_request(self, caplog, allowable_ppr, well_spaced_attempts, populate_redis):
        """
    ...
    def test_approve_last_remaining_quota(self, caplog, allowable_ppr, well_spaced_attempts, populate_redis):
        """
    ...
    def test_approve_last_with_multiple_recipients(self, caplog, groupsend_ppr, well_spaced_attempts, populate_redis):
        """
        ...
        populate_redis( groupsend_ppr.sender, 100, well_spaced_attempts(100-recipient_count) )
        policy = OutboundQuotaPolicy()
        assert policy.approve_policy_request( groupsend_ppr )

    def test_approve_underquota_within_margin(self, caplog, multisend_ppr_factory, well_spaced_attempts, populate_redis):
        """
    ...
    def test_deny_policy_request(self, overquota_ppr, well_spaced_attempts, populate_redis):
        """
    ...
    def test_deny_when_too_many_recipients(self, multisend_ppr_factory, well_spaced_attempts, populate_redis):
        """
    ...
    def test_deny_overquota_within_margin(self, caplog, groupsend_ppr, well_spaced_attempts, populate_redis):
        """
    ...
    def test_return_cached_instance_approval(self,
            allowable_ppr,
    ...
    def test_approve_policy_request_for_uncached(self,
    ...
    def test_deny_policy_request_for_undefined(self,
            undefined_ppr,
            populated_database_fixture,
            testing_policy
    ):
...
class Test_SenderDomainAuthPolicy():
    ...
    def test_sender_domain_key( self, allowable_ppr ):
        ...
        assert redis_key == f"sda:{allowable_ppr.sasl_username}:{allowable_ppr.sender.split('@')[1]}"
    ...
    def test_cached_authed_user( self, monkeypatch, sda_allowable_ppr, testing_policy_sda ):
