...
from chapps.tests.test_policy.conftest import _spf_actions, _spf_results, _auto_query_param_list, idfn
...
class Test_SPFEnforcementPolicy():
    ...
    # @pytest.mark.xfail
    def test_passing_emails_get_prepend(self, caplog, monkeypatch, passing_spf_query, allowable_ppr):
    ...
    def test_failing_emails_get_reject(self, caplog, monkeypatch, failing_spf_query, allowable_ppr):
        caplog.set_level( logging.DEBUG )
        with monkeypatch.context() as m:
    ...
    # @pytest.mark.xfail
    def test_no_helo_passing_mf_gets_prepend(self, caplog, monkeypatch, no_helo_passing_mf, allowable_ppr):
        caplog.set_level( logging.DEBUG )
        with monkeypatch.context() as m:
    ...
    def test_passing_helo_failing_mf_gets_reject(self, caplog, monkeypatch, passing_helo_failing_mf, allowable_ppr):
        caplog.set_level( logging.DEBUG )
        with monkeypatch.context() as m:
    ...
    @pytest.mark.parametrize( 'auto_spf_query, expected_result',
                              auto_query_param_list,
                              indirect=['auto_spf_query'], ids=idfn )
    def test_all_spf_query_result_permutations(self, caplog, monkeypatch,
                                               clear_redis_grl,
                                               auto_spf_query, expected_result, allowable_ppr):
        caplog.set_level( logging.DEBUG )
        with monkeypatch.context() as m:
