...
class CHAPPSModel( BaseModel ):
    ...
    @classmethod
    def joined_select( model, filter_model, *, where: List[ str ], window=(0,1000), order = 'm.id' ):  ### model table: m, join table: j
    ...
    @classmethod
    def double_joined_select( model, filter_model, where: List[ str ], window=(0,1000), order = 'm.id' ):  ### model table: m, filter_model table: f, join table: j
        """Build a generalized LEFT JOIN instruction, to say, get all domains for a user"""
        mm, fm = model.__name__.lower(), filter_model.__name__.lower()
        mt = mm + 's'
        mid = mm + "_id"
