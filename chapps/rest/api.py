...
async def create_user(
    ...
    if quota_id:
        ...
        if quota_resp.response is not None:
            ...
            return ErrorResp.send(
                None,
                error="integrity",
                message=f"There is no quota with id = {quota_id}",
            )
    if domain_ids:
        domains_resp = await list_domains("%", 0, 1000, domain_ids)
        if domains_resp.response is not None and len(domains_resp.response) == len(
            domain_ids
        ):
            domain_link_query = (
                f"INSERT INTO domain_user (domain_id, user_id) VALUES ( ? , @newuid );"
            )
            ...
            return ErrorResp.send(
                None,
                error="integrity",
                message=f"One or more of the specified domains does not exist",
            )
    new_user = None
    with pca.adapter_context() as cur:
        try:
            ...
            return ErrorResp.send(
                None, error="integrity", message="A user with that name already exists."
            )
        except Exception as e:
            logger.exception("Attempting to create a user.")
        if new_user is None:
            return ErrorResp.send(
                None, error="internal", message="No user was created."
            )
        if quota_link_query:
            try:
...
async def delete_user(
    ...
    )  # this is the prikey, no need to limit
    with pca.adapter_context() as cur:
        cur.execute(select)
