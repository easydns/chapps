        ...
        if not get_user:
            ...
            def get_user(ppr):
                for (
                    ...
                    qk_list
                ):  # avoiding a list-comp here because I'd rather not evaluate getattr any more times than I have to
                    user = getattr(ppr, k, None)
                    if user and user != "None":
                        logger.debug(f"Selecting quota-identifier {user} from key {k}")
                        return user
                raise ValueError(
                    f"None of the following keys had values in the provided PPR: {qk_list}"
                )
